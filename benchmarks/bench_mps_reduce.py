...
def run_fixed(args):
    ...
    cases = [
        ...
        ("var_mean dim=1", (128, 256), "torch.var_mean(x, dim=1); torch.mps.synchronize()"),
        ("std_mean dim=1", (128, 256), "torch.std_mean(x, dim=1); torch.mps.synchronize()"),
...
def run_large(args):
    ...
    cases = [
        ...
        ("argmax dim=1", (1024, 4096), "torch.argmax(x, dim=1); torch.mps.synchronize()"),
...
def run_variable(args):
    ...
    for op_name, op_fn_str in [("var dim=0", "torch.var(x, dim=0)"),
        ...
        result = bench(stmt, {"tensors": tensors, "torch": torch},
                       min_run_time=args.min_time)
        per_call = result.median * 1e6 / len(shapes)
        iqr_per = result.iqr * 1e6 / len(shapes)
        print(f"{op_name:<24} 20 shapes     {per_call:>8.1f} +/- {iqr_per:>5.1f} us/call")
